...
function NewViolationPageContent() {
  ...
  async function onSubmit(data: ViolationFormValues) {
    ...
    try {
        ...
        toast({ title: t('common.success'), description: t('violations.toasts.saveSuccess') });
        router.push('/violations');

    } catch (error) {
        console.error('Failed to save violation:', error);
        toast({ variant: 'destructive', description: t('violations.toasts.saveError') });
    } finally {
  ...
  if (!isDataLoaded) {
      return (
         <div className="flex flex-col gap-6">
            <PageHeader title={t('violations.form.newTitle')} />
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 flex flex-col gap-6">
                  ...
                  <div className="flex items-center justify-center w-full">
                    <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-card hover:bg-muted">
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p className="mb-2 text-sm text-muted-foreground"><span className="font-semibold">{t('violations.form.dropzoneHint')}</span></p>
            ...
            <div className="flex justify-end gap-2">
                <Button variant="outline" type="button" onClick={() => {form.reset(); setBranchDetails(null); router.back();}}>{t('common.cancel')}</Button>
